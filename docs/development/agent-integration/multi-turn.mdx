...
Use this pattern when you want to stream partial output to users while keeping one clean assistant message in conversation history.

{/* <!-- embedme examples/agent-integration/multi-turn/streaming-buffered-history/src/streaming_buffered_history/agent.py--> */}
```python
```
...
- Use **stream + buffer + one store** when your agent emits multiple partial chunks.

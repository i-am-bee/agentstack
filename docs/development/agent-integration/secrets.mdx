...
Check if secrets are provided and request them dynamically if needed. Use `await secrets.request_secrets()` to ask for secrets during runtime if they weren't provided beforehand. Implement appropriate fallback behavior when secrets are not available.
</Step>
</Steps>

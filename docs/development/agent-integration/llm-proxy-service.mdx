...
When building AI agents, one of the first requirements is connecting to a Large Large Model (LLM). Agent Stack provides built-in, OpenAI-compatible LLM inference that is model and provider agnostic.
...
async def my_agent(input: Message, llm: Any) -> None:

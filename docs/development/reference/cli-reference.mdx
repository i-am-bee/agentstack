...
* `-cid, --client-id TEXT`: Client ID for authentication, acquired form env if not supplied
* `-cs, --client-secret TEXT`: Client secret for authentication, acquired form env if not supplied
* `-md, --metadata TEXT`: Metadata as JSON string
* `-mp, --match-preset`: Use preset configuration for given url if it exists  [default: True]
* `--help`: Show this message and exit.
...
Manage MCP servers and toolkits. Some commands are [Admin only].

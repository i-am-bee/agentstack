...
Create a context token for the user session so the agent can access platform services. This token is required for the A2A client. See **[Permissions and Tokens](./permissions-and-tokens)** for how to generate a context token and scope permissions.

## 2. Create an A2A client

Use the A2A client factory and the AgentStack authenticated fetch helper. `createAuthenticatedFetch` attaches the `Authorization` header to every request so the A2A client can load the agent card and open JSON RPC streams with authentication. That is the only reason for the custom factory.
...
## 3. Resolve agent card demands

Once you have the client, read the agent card and resolve its demands.

Demands map to A2A extensions. See **[Agent Requirements](./agent-requirements)** for extension definitions and helper usage.
...
## 4. Send a message stream and handle updates
...
for await (const event of stream) {
  ...
  if (event.kind === "message") {
    // Non streaming agents may emit a final message event
    // Render event.parts and event.metadata
  }
}
```

...
Agent Stack supports rate limiting using Redis. This feature is disabled by default. To enable rate limiting, you must enable Redis, either by using the built-in instance or connecting to an external one.

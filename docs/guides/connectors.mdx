---
title: "Connectors"
description: "Create and manage data connections to extenal services"
---
...
1. **Create**: Client creates a connector by calling `POST /connectors` with MCP server URL.
2. **Connect**: Client initiates connection by calling `POST /connectors/{id}/connect`. If the MCP server requires user authorization, the response will contain an authorization URL.

...
<Steps>
<Step title="Install WSL (required)">

Agent Stack requires WSL to run on Windows.  

1. Check if WSL is installed by running in PowerShell:

```powershell
wsl --status
```

2. If WSL is not installed, run:

```powershell
wsl.exe --install Ubuntu
```

3. Reboot your PC to complete the installation.

4. After reboot, confirm installation with `wsl --status`.
</Step>

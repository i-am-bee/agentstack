...
# Add extensions to function signature
async def agent_function(
    message: Message,
    context: RunContext,
    trajectory: Annotated[TrajectoryExtensionServer, TrajectoryExtensionSpec()],
    ...
    # Trajectory updates during processing
    yield trajectory.trajectory_metadata(
        title="Processing Message",
        content=f"Processing: '{user_message}'"
    )
    ...
    if last_step and last_step.tool is not None:
        yield trajectory.trajectory_metadata(
            ...
            content=f"Executed {last_step.tool.name} with input: {last_step.input}"
        )
    
    # Search-specific trajectory
    ...
    response_text = "Your response with [cited sources](https://example.com)"
    
    # Extract and yield citations
    citations, clean_text = extract_citations(response_text, search_results)
    
    # Yield the clean response text
    yield clean_text
    
    # Yield citations if any were found
    if citations:
        yield citation.citation_metadata(citations=citations)
    
    # Final completion trajectory
    yield trajectory.trajectory_metadata(
        title="Completion",

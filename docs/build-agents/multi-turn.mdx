...
async def example_agent(input: Message, context: RunContext):
    ...
    # Load all previous messages from conversation history
    history = [
    ...
    print(f"Found {len(history)} previous messages")
    
    # Your agent logic here - you can now reference previous messages
    yield AgentMessage(text=f"Hello! I can see we have {len(history)} previous messages in our conversation.")
...
The `context.store.load_history()` method returns an async iterator containing all previous items in the conversation. This can include:

- **A2A Messages**: Both user and assistant messages from the conversation

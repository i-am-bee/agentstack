...
yield trajectory.trajectory_metadata(
    content="Analyzed 3/8 results",
    group_id="websearch"
)
yield trajectory.trajectory_metadata(
    content="Analyzed 8/8 results",
    group_id="websearch"
)
...
Creates trajectory metadata that can be yielded to show the agent's reasoning process.

**Parameters:**

- `title` (str, optional): The heading for this trajectory step. Used as the section title in the UI.
- `content` (str, optional): Detailed description of what the agent is doing or thinking.
- `group_id` (str, optional): Identifier to group multiple trajectory updates together. Each update with the same `group_id` replaces the previous content (and title, if provided) in the UI.

**Returns:** Metadata object that should be yielded to the client.
...
metadata = trajectory.trajectory_metadata(
    ...
    content="Examining user input for key patterns...",
    group_id="analysis"
)
yield metadata
```

...
description: "Learn how to inject MCP servers from the client on per-task basis"
---

In scenarios where it is necessary to dynamically change the MCP server(s) used by an agent, the Agent Stack provides an MCP extension that allows clients to specify MCP server details at runtime.
...
<Step title="Configure your MCP request">
  Specify which MCP server(s) your agent prefer(s).
</Step>
...
async def example_agent(
    ...
    """Agent that uses MCP to list tools"""
...
In the usual scenario where the specified MCP server requires OAuth authentication, the MCP extension automatically leverages the OAuth extension if activated. Technically, it does exactly what is described in [Authorize MCP with OAuth](mcp-oauth).

## Usage with Connectors

To further improve user experience, one can use [Connectors](../guides/connectors) and have the Agent Stack platform automatically manage access tokens, hence effectively skipping the OAuth flow during agent execution. This is achieved by injecting a connector instead of the actual MCP server that backs the connector.

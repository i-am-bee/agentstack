...
2. **Connect**: Client initiates connection by calling `POST /api/v1/connectors/{id}/connect`.
   ...
   - For token-based authentication: Provide an `authToken` in the connect request body (see [Authentication](#authentication))
...
4. **Complete**: Platform exchanges the authorization code for access and refresh tokens (OAuth) or stores the provided auth token. Once completed, the connector is in `connected` state and ready to be used.
...
- **Managed stdio MCP servers**: Token is injected as an environment variable in the container (requires `authTokenEnvName` in preset configuration)
    ...
    - url: "mcp+stdio://example-mcp"
      ...
      stdio:
        ...
        env:  # Optional: additional environment variables
          LOG_LEVEL: "info"
        authTokenEnvName: "API_TOKEN"  # Optional: env var name to inject auth token
```
...
5. If an `authToken` was provided in the connect request and the preset defines `authTokenEnvName`, injects the token as an environment variable

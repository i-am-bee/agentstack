...
While both are essential parts of the BeeAI initiative under the Linux Foundation, the similar naming has led to some
...
Agent Stack is a self-hostable stack for deploying and sharing AI agents. It solves a specific problem: once you've
built an agent (using any framework or even custom code), how do you actually deploy it somewhere people can use it?
...
- **Infrastructure services** that your agents can use:
  - Runtime-configurable LLM providers - switch between Anthropic, watsonx.ai, OpenAI, Ollama, etc. without changing
    agent code
...
- **A HELM Chart** that allows anyone to configure the platform as they want (plug in your own s3 compatible storage,
  managed database, authentication, etc.)

The SDK extensions work with any framework — if your agent is built on A2A, it can use these services regardless of
whether you built it with LangGraph, CrewAI, BeeAI Framework, or something custom.

**The workflow:**

1. Build with any framework (LangGraph, CrewAI, etc.)
2. Deploy with Agent Stack using a single CLI command
3. Share via web UI with chat interface

Agent Stack is designed to work flexibly with your infrastructure, your tools, your framework, and your LLM providers.
...
Agent Stack provides self-hostable infrastructure that works with any A2A-compliant agent, giving teams flexibility
while the agent market is still figuring itself out.
...
```
from agentstack_sdk import Server  # new
```

**All agent images will need to be rebuilt to apply the appropriate change.**

## What’s Next

Agent Stack is in incubation—this means we're actively exploring what agent deployment infrastructure should look like
and learning from real-world usage. We're focused on:
...
- Improving the development workflow. The CLI and web UI should get out of your way. We're gathering feedback on what
  works and what doesn't, and iterating based on real usage.
- Building ecosystem connections. As more frameworks adopt A2A, we want Agent Stack to work seamlessly with them. We're
  also exploring better ways to discover and share agents.
- Understanding real needs. Incubation means we don't have all the answers yet. We're learning what teams actually need
  from deployment infrastructure—what's essential, what's nice-to-have, and what we got wrong.
...
Reach out to us on [Discord](https://discord.gg/NradeA6ZNF) or [GitHub](https://github.com/i-am-bee/agentstack)—we’re
here to help make the transition smooth.

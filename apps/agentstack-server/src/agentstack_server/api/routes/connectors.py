...
async def mcp(
    ...
    request: Request,
    connector_service: ConnectorServiceDependency,
    _: Annotated[AuthorizedUser, Depends(RequiresPermissions(connectors={"read"}))],
):
    response = await connector_service.mcp_proxy(connector_id=connector_id, request=request)

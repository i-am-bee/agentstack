    ...
    try:
        if detect_driver() == "lima":
            ...
            for line in result.stdout.decode().split("\n"):
                if line:
                    status_data = pydantic.TypeAdapter(dict[str, str]).validate_json(line)
                    if status_data.get("name") == vm_name:
...
async def exec_in_vm(vm_name: str, command: list[str]) -> None:
    ...
    if detect_driver() == "lima":
        await anyio.run_process(
            [get_limactl(), "shell", f"--tty={sys.stdin.isatty()}", vm_name, "--", *command],
            check=False,
            stdin=sys.stdin,
            stdout=sys.stdout,
            stderr=sys.stderr,
...
async def create_vm(vm_name: str) -> None:
    ...
    if detect_driver() == "lima":
        ...
        if not current_status:
            ...
            await run_command([get_limactl(), "--tty=false", "start", vm_name], "Starting up", env=lima_env, cwd="/")
        else:
...
async def import_images(vm_name: str, platform: typing.Literal["k3s", "microshift"], *tags: str) -> None:
    ...
    try:
        ...
        await run_in_vm(
            vm_name, cmd, f"Importing image{'' if len(tags) == 1 else 's'} {', '.join(tags)} into Agent Stack platform"
        )
    finally:
...
async def deploy(
    ...
    if image_pull_mode in {ImagePullMode.guest, ImagePullMode.hybrid}:
        for image in loaded_images - images_to_import_from_host:
            async for attempt in AsyncRetrying(stop=stop_after_attempt(5)):
                with attempt:
                    cmd = ["k3s", "ctr", "image", "pull", image] if platform == "k3s" else ["crictl", "pull", image]
                    await run_in_vm(
                        vm_name,
                        cmd,
    ...
    if shas_guest_before and (
        ...
        for pod in json.loads(
            ...
            if any(
                cs.get("imageID", "") in replaced_digests for cs in pod.get("status", {}).get("containerStatuses", [])
            ):
                await run_in_vm(
                    vm_name,
                    [
...
async def start(
    ...
    with verbosity(verbose):
        ...
        await deploy(
            vm_name,
            await install_tools(vm_name),
            set_values_list,
            pathlib.Path(values_file) if values_file else None,
            image_pull_mode,
        )
...
async def import_image_cmd(
    ...
    with verbosity(verbose):
        if (await get_vm_status(vm_name)) != "running":
            ...
            sys.exit(1)
        await import_images(vm_name, await detect_platform(vm_name), tag)

...
async def _test_client_side_build_skip_extraction():
    with (
        patch("agentstack_cli.commands.build.run_command") as mock_run_command,
        ...
        mock_async_retrying.return_value = mock_retry_instance

        async def async_iter():
            yield MagicMock()  # attempt context

        mock_retry_instance.__aiter__.side_effect = async_iter
        ...
        await client_side_build(context=".", import_image=False, extract_agent_card=False, verbose=True)
...
async def _test_client_side_build_default():
    with (
        ...
        # The attempt context manager yielded by AsyncRetrying
        mock_attempt_ctx = MagicMock()
        mock_attempt_ctx.__enter__.return_value = None
        mock_attempt_ctx.__exit__.return_value = None

        async def async_iter():
            yield mock_attempt_ctx
        ...
        # Mock AsyncClient
        # When called as AsyncClient(), it returns an instance
        mock_client_instance = MagicMock()
        mock_async_client.return_value = mock_client_instance

        # Mock the context manager for that instance: async with AsyncClient() as client:
        # client will be the return value of __aenter__
        mock_client_context = MagicMock()
        mock_client_instance.__aenter__ = AsyncMock(return_value=mock_client_context)
        ...
        mock_client_context.get.side_effect = async_get

        # Call with defaults (extract_agent_card should be True by default)
        await client_side_build(context=".", import_image=False, verbose=True)

        # Verify open_process was called
        assert mock_open_process.called, "Container should have been launched"

        # Verify AsyncClient was used
        assert mock_async_client.called, "AsyncClient should have been instantiated"
                ...
                for arg in command:
                    if arg.startswith("--label=beeai.dev.agent.json="):
                        ...
                        if len(parts) >= 3:
                            ...
                            if card and card.get("test") == "card":
                                found_label = True
